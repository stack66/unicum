    ...
    for i, url in enumerate(urls):
        ...
        try:
            ...
            if err == 10054:
                    #reconnect()
                    #r = session.get(url, timeout=(3,3))
                    with open('!con-error.log','a') as f:
                        f.write(f'{err}\n{url}')
        ...
        try:
            ta = tp.xpath('//h1/a/text()')[0].strip().replace('\xa0','')   # id auto
        except:
            with open('!con-error.log','a') as f:
                f.write(f'url {url}\n goto  {goto}\n')
            continue
        ...
        #dt = dts.apply(lambda x: int(pd.Timestamp(x).round(freq='T').timestamp())) # !!! timezone +7
        ...
        if len(lname)<22:
            ...
            for i, v in leftover.items():
                jleft += f'"{lname[i-1]}":{v},'
            left = '{'+jleft[:-1]+'}'
        query = f'insert into {tab} (id,tdate,quantity,coin,leftover,status) values(?,?,?,?,?,?)'
        zipped = (ta,dt,jsq,jsc,left,status[i])
